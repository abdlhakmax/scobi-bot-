...
console.log('🚩 Connection already intialized...');
} else {
global.conns = [];
}*/
...
global.authFile = `elven/session`
global.authFileJB = `elven/subbots`
...
do {
opcion = await question(colores('Choose an option:\n') + opcionQR('1. Scan QR Code\n') + opcionTexto('2. Pair using 8Digits code\n--> '))
if (!/^[1-2]$/.test(opcion)) {
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['Elven Bot - MD', 'Edge', '2.0.0'] : methodCodeQR ? ['Elven Bot - MD', 'Edge', '2.0.0'] : ['Ubuntu', 'Edge', '110.0.1587.56'],
auth: {
creds: state.creds,
...
console.log(chalk.black(chalk.bgGreen(`👑 BINDING CODE 👑`)), chalk.bold.white(chalk.white(codeBot)))
}, 2000)
}}}
}
...
console.log('🚩 ElvenBot has no SubBot attached.');
return;
}
...
const mainBotAuthFile = 'StorageSession';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🚩 ElvenBot conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🍭 Error al iniciar ElvenBot: `, error))
}
})();*/
